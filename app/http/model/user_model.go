...
type User struct {
	...
	PasswordSms   string `gorm:"column:password_sms" json:"-"`          // 密码
	...
	EmailV        string `gorm:"column:email_v" json:"-"`
	Digests       int    `gorm:"column:digests" json:"-"`
	Digests3      int    `gorm:"column:digests_3" json:"-"`
	Signature     string `gorm:"column:signature" json:"-"` // 用户签名
	//Group    `json:"group" gorm:"foreignKey:gid;references:gid"`
	//ThreadList    []Thread `json:"thread_list" gorm:"foreignKey:uid;references:uid"`
}
