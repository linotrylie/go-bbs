...
func (repo *friendlinkRepository) First(friendlink *model.Friendlink, preload []string) (e error) {
	...
	db := global.DB.Table(friendlink.TableName()).Where(friendlink.Location())
	if preload != nil {
...
func (repo *friendlinkRepository) SaveInRedisByKey(redisKey string, data string) {
...
func (repo *friendlinkRepository) GetDataByWhereMap(friendlink *model.Friendlink, where map[string]interface{}, preload []string) (e error) {
	...
	db := global.DB.Table(friendlink.TableName()).Where(where)
	if preload != nil {
		for _, v := range preload {
			db = db.Preload(v)
		}
