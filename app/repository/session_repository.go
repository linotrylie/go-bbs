...
func (repo *sessionRepository) First(session *model.Session) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	//先查询redis缓存
	err := repo.FindInRedis(session)
	if err != nil && err != redis.Nil {
		return err
	}
	result := global.DB.Table(session.TableName()).Where(session.Location()).First(session)
	e = result.Error
	if result.Error != nil {
...
func (repo *sessionRepository) SaveInRedisByKey(redisKey string, data string) (e error) {
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
		}
...
func (repo *sessionRepository) GetDataListByWhereMap(query map[string]interface{}) (list []*model.Session, e error) {
	now := time.Now()
...
func (repo *sessionRepository) GetDataListByWhere(query string, args []interface{}) (list []*model.Session, e error) {
	now := time.Now()
	session := &model.Session{}
	defer func() {
		if e != nil {
...
func (repo *sessionRepository) GetDataByWhereMap(session *model.Session, where map[string]interface{}) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	db := global.DB.Table(session.TableName()).Where(where).First(session)
	e = db.Error
	if e != nil {
		return
	}
