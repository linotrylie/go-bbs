...
func (repo *xnVoteRepository) Update(xnVote *model.XnVote) (rowsAffected int64, e error) {
	...
	result := global.DB.Table(xnVote.TableName()).Where(xnVote.Location()).Updates(updateValues)
	if result.Error != nil {
		return
	}
	//更新完成后，重新缓存
...
func (repo *xnVoteRepository) First(xnVote *model.XnVote) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	//先查询redis缓存
	err := repo.FindInRedis(xnVote)
	if err != nil && err != redis.Nil {
		return err
	}
	result := global.DB.Table(xnVote.TableName()).Where(xnVote.Location()).First(xnVote)
	e = result.Error
	if result.Error != nil {
...
func (repo *xnVoteRepository) DeleteByLocation(xnVote *model.XnVote) (rowsAffected int64, e error) {
	...
	result := global.DB.Table(xnVote.TableName()).Where(xnVote.Location()).Unscoped().Delete(xnVote)
	if result.Error != nil {
		return
	}
	repo.DeleteInRedis(xnVote)
...
func (repo *xnVoteRepository) SaveInRedisByKey(redisKey string, data string) (e error) {
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
		}
...
func (repo *xnVoteRepository) GetDataListByWhereMap(query map[string]interface{}) (list []*model.XnVote, e error) {
	now := time.Now()
	xnVote := &model.XnVote{}
	defer func() {
		if e != nil {
	...
	db := global.DB.Table(xnVote.TableName()).Where(query)
...
func (repo *xnVoteRepository) GetDataListByWhere(query string, args []interface{}) (list []*model.XnVote, e error) {
	now := time.Now()
	xnVote := &model.XnVote{}
	defer func() {
		if e != nil {
...
func (repo *xnVoteRepository) GetDataByWhereMap(xnVote *model.XnVote, where map[string]interface{}) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	db := global.DB.Table(xnVote.TableName()).Where(where).First(xnVote)
	e = db.Error
	if e != nil {
		return
	}
