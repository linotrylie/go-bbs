...
func (repo *xnVoteInfoRepository) Insert(xnVoteInfo *model.XnVoteInfo) (rowsAffected int64, e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
			global.Prome.OrmWithLabelValues(xnVoteInfo.TableName(), "Insert", e, now)
		}
	}()
	result := global.DB.Create(xnVoteInfo)
	e = result.Error
	if e != nil {
		return
	}
...
func (repo *xnVoteInfoRepository) Update(xnVoteInfo *model.XnVoteInfo) (rowsAffected int64, e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
			global.Prome.OrmWithLabelValues(xnVoteInfo.TableName(), "Update", e, now)
		}
	}()
	if len(xnVoteInfo.Location()) == 0 {
		return 0, errors.New("无更新条件！")
	}
	updateValues := xnVoteInfo.GetChanges()
...
func (repo *xnVoteInfoRepository) First(xnVoteInfo *model.XnVoteInfo, preload []string) (e error) {
	...
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
			global.Prome.OrmWithLabelValues(xnVoteInfo.TableName(), "First", e, now)
		}
	}()
	if len(xnVoteInfo.Location()) == 0 {
...
func (repo *xnVoteInfoRepository) DeleteByLocation(xnVoteInfo *model.XnVoteInfo) (rowsAffected int64, e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
			global.Prome.OrmWithLabelValues(xnVoteInfo.TableName(), "DeleteByLocation", e, now)
		}
	}()
	if len(xnVoteInfo.Location()) == 0 {
		return 0, errors.New("无更新字段！")
...
// 事务
func (repo *xnVoteInfoRepository) TransactionExecute(fun func() error, opts ...*sql.TxOptions) (e error) {
...
func (repo *xnVoteInfoRepository) SaveInRedis(xnVoteInfo *model.XnVoteInfo) (e error) {
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
		}
	...
	resByte, e := json.Marshal(xnVoteInfo)
	if e != nil {
		return e
	}
...
func (repo *xnVoteInfoRepository) FindInRedis(xnVoteInfo *model.XnVoteInfo) (e error) {
	defer func() {
...
func (repo *xnVoteInfoRepository) SaveInRedisByKey(redisKey string, data string, timeout int) {
...
func (repo *xnVoteInfoRepository) GetDataListByWhereMap(query map[string]interface{}, preload []string) (list []*model.XnVoteInfo, e error) {
	...
	redisKey := xnVoteInfo.TableName() + "_list_" + strconv.Itoa(repo.Pager.Page) + "_" + strconv.Itoa(repo.Pager.PageSize) + "_" + str
...
func (repo *xnVoteInfoRepository) GetDataListByWhere(query string, args []interface{}, preload []string) (list []*model.XnVoteInfo, e error) {
	...
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
			global.Prome.OrmWithLabelValues(xnVoteInfo.TableName(), "GetDataListByWhere", e, now)
		}
	}()
	var str string
	if repo.Pager.FieldsOrder != nil {
...
func (repo *xnVoteInfoRepository) GetTotalPage(db *gorm.DB) (e error) {
