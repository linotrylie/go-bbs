...
func (repo *userRepository) First(user *model.User) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	//先查询redis缓存
	err := repo.FindInRedis(user)
	if err != nil && err != redis.Nil {
		return err
	}
	result := global.DB.Table(user.TableName()).Where(user.Location()).First(user)
	e = result.Error
	if result.Error != nil {
...
func (repo *userRepository) SaveInRedisByKey(redisKey string, data string) (e error) {
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
		}
...
func (repo *userRepository) GetDataListByWhereMap(query map[string]interface{}) (list []*model.User, e error) {
	now := time.Now()
	user := &model.User{}
	defer func() {
		if e != nil {
...
func (repo *userRepository) GetDataListByWhere(query string, args []interface{}) (list []*model.User, e error) {
	now := time.Now()
	user := &model.User{}
	defer func() {
		if e != nil {
...
func (repo *userRepository) GetDataByWhereMap(user *model.User, where map[string]interface{}) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	db := global.DB.Table(user.TableName()).Where(where).First(user)
	e = db.Error
	if e != nil {
		return
	}
