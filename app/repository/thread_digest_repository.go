...
func (repo *threadDigestRepository) Update(threadDigest *model.ThreadDigest) (rowsAffected int64, e error) {
	...
	repo.First(threadDigest)
	e = result.Error
	rowsAffected = result.RowsAffected
	return
}

func (repo *threadDigestRepository) First(threadDigest *model.ThreadDigest) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	//先查询redis缓存
	err := repo.FindInRedis(threadDigest)
	if err != nil && err != redis.Nil {
		return err
	}
	result := global.DB.Table(threadDigest.TableName()).Where(threadDigest.Location()).First(threadDigest)
	e = result.Error
	if result.Error != nil {
...
func (repo *threadDigestRepository) SaveInRedisByKey(redisKey string, data string) (e error) {
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
		}
...
func (repo *threadDigestRepository) GetDataListByWhereMap(query map[string]interface{}) (list []*model.ThreadDigest, e error) {
	now := time.Now()
...
func (repo *threadDigestRepository) GetDataListByWhere(query string, args []interface{}) (list []*model.ThreadDigest, e error) {
	now := time.Now()
	threadDigest := &model.ThreadDigest{}
	defer func() {
		if e != nil {
...
func (repo *threadDigestRepository) GetDataByWhereMap(threadDigest *model.ThreadDigest, where map[string]interface{}) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	db := global.DB.Table(threadDigest.TableName()).Where(where).First(threadDigest)
	e = db.Error
	if e != nil {
		return
	}
