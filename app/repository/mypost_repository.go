...
func (repo *mypostRepository) First(mypost *model.Mypost) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	//先查询redis缓存
	err := repo.FindInRedis(mypost)
	if err != nil && err != redis.Nil {
		return err
	}
	result := global.DB.Table(mypost.TableName()).Where(mypost.Location()).First(mypost)
	e = result.Error
	if result.Error != nil {
...
func (repo *mypostRepository) SaveInRedisByKey(redisKey string, data string) (e error) {
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
		}
...
func (repo *mypostRepository) GetDataListByWhereMap(query map[string]interface{}) (list []*model.Mypost, e error) {
	now := time.Now()
	mypost := &model.Mypost{}
	defer func() {
		if e != nil {
...
func (repo *mypostRepository) GetDataListByWhere(query string, args []interface{}) (list []*model.Mypost, e error) {
	now := time.Now()
	mypost := &model.Mypost{}
	defer func() {
		if e != nil {
...
func (repo *mypostRepository) GetDataByWhereMap(mypost *model.Mypost, where map[string]interface{}) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	db := global.DB.Table(mypost.TableName()).Where(where).First(mypost)
	e = db.Error
	if e != nil {
		return
	}
