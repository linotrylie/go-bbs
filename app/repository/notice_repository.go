...
func (repo *noticeRepository) Update(notice *model.Notice) (rowsAffected int64, e error) {
	...
	result := global.DB.Model(notice).Updates(updateValues)
	e = result.Error
	if e != nil {
		return 0, e
	}
	...
	repo.First(notice, []string{})
	e = result.Error
	rowsAffected = result.RowsAffected
	return
}

func (repo *noticeRepository) First(notice *model.Notice, preload []string) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
...
func (repo *noticeRepository) DeleteByLocation(notice *model.Notice) (rowsAffected int64, e error) {
	...
	result := global.DB.Table(notice.TableName()).Unscoped().Delete(notice)
	e = result.Error
	if e != nil {
		return 0, e
	}
...
func (repo *noticeRepository) SaveInRedisByKey(redisKey string, data string) {
...
func (repo *noticeRepository) GetDataListByWhereMap(query map[string]interface{}, preload []string) (list []*model.Notice, e error) {
	...
	db := global.DB.Model(notice).Where(query)
	if preload != nil {
		for _, v := range preload {
			db.Preload(v)
		}
...
func (repo *noticeRepository) GetDataByWhereMap(notice *model.Notice, where map[string]interface{}, preload []string) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
