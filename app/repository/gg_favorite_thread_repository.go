...
func (repo *ggFavoriteThreadRepository) Update(ggFavoriteThread *model.GgFavoriteThread) (rowsAffected int64, e error) {
	...
	repo.First(ggFavoriteThread)
	e = result.Error
	rowsAffected = result.RowsAffected
	return
}

func (repo *ggFavoriteThreadRepository) First(ggFavoriteThread *model.GgFavoriteThread) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	//先查询redis缓存
	err := repo.FindInRedis(ggFavoriteThread)
	if err != nil && err != redis.Nil {
		return err
	}
	result := global.DB.Table(ggFavoriteThread.TableName()).Where(ggFavoriteThread.Location()).First(ggFavoriteThread)
	e = result.Error
	if result.Error != nil {
...
func (repo *ggFavoriteThreadRepository) SaveInRedisByKey(redisKey string, data string) (e error) {
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
		}
...
func (repo *ggFavoriteThreadRepository) GetDataListByWhereMap(query map[string]interface{}) (list []*model.GgFavoriteThread, e error) {
	now := time.Now()
...
func (repo *ggFavoriteThreadRepository) GetDataListByWhere(query string, args []interface{}) (list []*model.GgFavoriteThread, e error) {
	now := time.Now()
	ggFavoriteThread := &model.GgFavoriteThread{}
	defer func() {
		if e != nil {
...
func (repo *ggFavoriteThreadRepository) GetDataByWhereMap(ggFavoriteThread *model.GgFavoriteThread, where map[string]interface{}) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	db := global.DB.Table(ggFavoriteThread.TableName()).Where(where).First(ggFavoriteThread)
	e = db.Error
	if e != nil {
		return
	}
