...
func (repo *inviteRepository) First(invite *model.Invite) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	//先查询redis缓存
	err := repo.FindInRedis(invite)
	if err != nil && err != redis.Nil {
		return err
	}
	result := global.DB.Table(invite.TableName()).Where(invite.Location()).First(invite)
	e = result.Error
	if result.Error != nil {
...
func (repo *inviteRepository) SaveInRedisByKey(redisKey string, data string) (e error) {
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
		}
...
func (repo *inviteRepository) GetDataListByWhereMap(query map[string]interface{}) (list []*model.Invite, e error) {
	now := time.Now()
	invite := &model.Invite{}
	defer func() {
		if e != nil {
...
func (repo *inviteRepository) GetDataListByWhere(query string, args []interface{}) (list []*model.Invite, e error) {
	now := time.Now()
	invite := &model.Invite{}
	defer func() {
		if e != nil {
...
func (repo *inviteRepository) GetDataByWhereMap(invite *model.Invite, where map[string]interface{}) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	db := global.DB.Table(invite.TableName()).Where(where).First(invite)
	e = db.Error
	if e != nil {
		return
	}
