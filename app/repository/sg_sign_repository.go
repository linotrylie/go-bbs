...
func (repo *sgSignRepository) First(sgSign *model.SgSign) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	//先查询redis缓存
	err := repo.FindInRedis(sgSign)
	if err != nil && err != redis.Nil {
		return err
	}
	result := global.DB.Table(sgSign.TableName()).Where(sgSign.Location()).First(sgSign)
	e = result.Error
	if result.Error != nil {
...
func (repo *sgSignRepository) SaveInRedisByKey(redisKey string, data string) (e error) {
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
		}
...
func (repo *sgSignRepository) GetDataListByWhereMap(query map[string]interface{}) (list []*model.SgSign, e error) {
	now := time.Now()
	sgSign := &model.SgSign{}
	defer func() {
		if e != nil {
...
func (repo *sgSignRepository) GetDataListByWhere(query string, args []interface{}) (list []*model.SgSign, e error) {
	now := time.Now()
	sgSign := &model.SgSign{}
	defer func() {
		if e != nil {
...
func (repo *sgSignRepository) GetDataByWhereMap(sgSign *model.SgSign, where map[string]interface{}) (e error) {
	now := time.Now()
	defer func() {
		if e != nil {
			global.LOG.Error(e.Error(), zap.Error(e))
	...
	db := global.DB.Table(sgSign.TableName()).Where(where).First(sgSign)
	e = db.Error
	if e != nil {
		return
	}
